...
function addProp(type){
  ...
  li.innerHTML='<span class="prop-item-name">'+(names[type]||type)+' '+propIdCnt+'</span><button class="prop-rm-btn" title="Sil">✕</button>';
...
function addSceneText(txt,size,yPos,color){
  ...
  li.innerHTML='<span class="text-item-label">'+txt+'</span><button class="text-rm-btn">✕</button>';
...
/* ═══ IŞIK SİSTEMİ ═══ */
const LT=new THREE.Vector3(0,1.15,0);let lCnt=0;const lightObjects={};let equipHidden=false;
...
function buildLight(sp){
  ...
  const sg=new THREE.Group();sg.add(new THREE.Mesh(new THREE.BoxGeometry(cfg.ps[0],cfg.ps[1],.06),new THREE.MeshStandardMaterial({color:0x222222,roughness:.6})));
  const pm=new THREE.MeshBasicMaterial({color:cfg.color,transparent:true,opacity:.81});sg.add(new THREE.Mesh(new THREE.PlaneGeometry(cfg.ps[0]*.86,cfg.ps[1]*.86),pm));
  sg.position.copy(pos);sg.lookAt(LT);scene.add(sg);
  const lh=Math.max(.05,pos.y);const leg=new THREE.Mesh(new THREE.CylinderGeometry(.025,.03,lh,8),new THREE.MeshStandardMaterial({color:0x161616,metalness:.5,roughness:.5}));leg.position.set(pos.x,lh/2,pos.z);
...
document.getElementById('btnShare').addEventListener('click',()=>{try{const b64=btoa(unescape(encodeURIComponent(getSceneJSON())));const url=location.href.split('#')[0]+'#scene='+b64;navigator.clipboard.writeText(url).then(()=>toast('Bağlantı kopyalandı ✓')).catch(()=>toast('URL oluşturuldu'));} catch(e){toast('Paylaşım hatası');}});
...
document.getElementById('btnExportDiag').addEventListener('click', async()=>{
  ...
  ctx.textAlign='left';
  ctx.fillText('Aydınlatma Stüdyosu - Rapor', 80, y);
  
  y+=100;
  ctx.font='bold 28px Arial';
  ctx.fillStyle='#555';
  ctx.fillText('IŞIK', 80, y);
  ctx.fillText(t('rep_dist'), 500, y);
  ctx.fillText('YATAY AÇI', 750, y);
  ctx.fillText('DİKEY AÇI', 1000, y);
  ctx.fillText('ŞİDDET', 1250, y);
  ctx.fillText('RENK (K)', 1500, y);
  ctx.fillText('DİĞER', 1750, y);
  ...
  activeLights.forEach(o=>{
    ...
    ctx.fillText(`Açı: ${sp}°, Yumuşatma: ${pn}`, 1750, y);
    y+=75;
  });
  ...
  const prodName=usingDefault?'urun':(document.getElementById('piName').textContent.replace(/\.[^.]+$/,'').replace(/\s+/g,'_')||'urun');
  a.download=prodName + '_' + t('rep_main').replace(/\s+/g, '_') + '.jpg';
  document.body.appendChild(a);a.click();document.body.removeChild(a);
  setTimeout(()=>URL.revokeObjectURL(url),3000);
  btn.textContent='RAPORU İNDİR (JPG)';
});

/* ═══ TAKE A SHOT ═══ */
document.getElementById('shotBtn').addEventListener('click',async function(){
  const btn=this;btn.classList.add('shooting');btn.innerHTML='<span class="shot-icon">⏳</span> Render alınıyor…';
  ...
  btn.innerHTML='<span class="shot-icon">💾</span> Kaydediliyor…';
  try{const fh=await window.showSaveFilePicker({suggestedName:fileName,types:[{description:'PNG',accept:{'image/png':['.png']}}]});const w=await fh.createWritable();await w.write(blob);await w.close();btn.innerHTML='<span class="shot-icon">✓</span> Kaydedildi!';}
  catch(e){if(e.name!=='AbortError'){const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=fileName;document.body.appendChild(a);a.click();document.body.removeChild(a);setTimeout(()=>URL.revokeObjectURL(url),3000);btn.innerHTML='<span class="shot-icon">✓</span> İndirildi!';}else{btn.innerHTML='<span class="shot-icon">📷</span> Take a Shot';btn.classList.remove('shooting');return;}}
  btn.classList.remove('shooting');setTimeout(()=>{btn.innerHTML='<span class="shot-icon">📷</span> Take a Shot';},2500);
});
...
function animate(t){
  ...
  if(lightPulseSpeed>0){pulseT+=dt*lightPulseSpeed;const pulse=.8+.2*Math.sin(pulseT*Math.PI*2);Object.values(lightObjects).forEach(o=>{if(o.spot.visible)o.spot.intensity=((o.curPow/100)*o.cfg.maxI)*(1-(o.params.diffOn?o.params.diffO*.7:0))*pulse;});}
...
document.getElementById('btnFPS').addEventListener('click',()=>{
  ...
  toast(fpsActive?'FPS sayacı açık':'FPS sayacı kapalı');
});
...
document.getElementById('gradClear').addEventListener('click',()=>{scene.background=new THREE.Color(0x121113);document.querySelectorAll('.bg-swatch').forEach(s=>s.classList.toggle('active',s.dataset.bg==='studio'));toast('Gradient kaldırıldı');});
...
function loadBGImage(file){
  ...
  reader.onload=(e)=>{
    ...
    },undefined,err=>toast('Resim yükleme hatası'));
  };
...
document.getElementById('bgImgRemoveBtn').addEventListener('click',()=>{
  ...
  bgImgZone.querySelector('.drop-label').innerHTML='<strong>Resim sürükle</strong> veya tıkla';
  toast('Arka plan kaldırıldı');
});
...
document.getElementById('floorImgRemoveBtn').addEventListener('click',()=>{
  ...
  toast('Zemin görseli kaldırıldı');
});
...
document.querySelectorAll('.bokeh-btn').forEach(b=>b.addEventListener('click',()=>{
  ...
  toast('Bokeh: '+b.textContent.trim());
}));
...
function refreshProfileSelect(){
  ...
  sel.innerHTML='<option value="">— Profil seç —</option>';
...
document.getElementById('saveLightProfileBtn').addEventListener('click',()=>{
  const name=prompt('Profil adı:');if(!name||!name.trim())return;
  const state=getSceneJSON();
  const profs=getLightProfiles();profs[name.trim()]=state;saveLightProfiles(profs);
  refreshProfileSelect();toast('💾 Profil kaydedildi: '+name.trim());
});
...
function analyzeRefImage(file){
  ...
  img.onload=()=>{
    ...
    refDominantColors.forEach(color=>{
      ...
      sw.addEventListener('click',()=>{scene.background=new THREE.Color(color);document.querySelectorAll('.bg-swatch').forEach(s=>s.classList.remove('active'));toast('Referans rengi uygulandı: '+color);});
      row.appendChild(sw);
    });
...
const origAnimate=window.__studioAnimate;
let waterAnimT=0;
function patchRenderLoop(){
  const origFn=animate;
  // Water vertex animation via time
  setInterval(()=>{
...
if(eyebrowEl)eyebrowEl.textContent='Studio Pro V11';
